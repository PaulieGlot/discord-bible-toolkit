...
def get_verse(section: str, book: str, chapter: int, verse: int):
    ...
    except FileNotFoundError: return "error fetching verse: chapter file `%s` does not exist.\n\n*Paulie says: check chapters.txt to see if it should!*\n" % filename
    for line in file:
        prefix = "%i: " % verse
        if line.startswith(prefix):
            return line
    return "error fetching verse: chapter file `%s` contains no verse numbered %i.\n\n*Paulie says: be sure you are using the same numbering system as this version, and that a translation has been supplied for the requested verse.*\n" % (filename, verse)
...
def get_verse_range(section: str, book: str, chapter: int, start_verse: int, end_verse: int):
    ...
    for line in file:
        ...
        if prefix > end_verse:
            break
    if text == "":
        return "error fetching verse range: chapter file `%s` contains no verses within the range %i - %i.\n\n*Paulie says: be sure you are using the same numbering system as this version, and that translations have been supplied for verses in the requested range.*\n" % (filename, start_verse, end_verse)
    return text
...
def get_section_name(book: str):
    ...
    return "error finding section name: book `%s` is not listed in chapters.txt.\n\n*Paulie says: make sure you're using the same book names as this version (these are case-sensitive), and that you have all planned books listed along with their chapter lengths!*" % book
